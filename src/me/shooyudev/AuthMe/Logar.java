  ...
  implements CommandExecutor
{
  public boolean onCommand(CommandSender sender, Command cmd, String arg2, String[] arg3){
    if (cmd.getName().equalsIgnoreCase("login")){
      ...
      if (arg3.length == 1){
        if (arg3[0].equalsIgnoreCase((String)AuthConfig.getConfig().config().get("Login." + p.getName().toLowerCase() + ".senha"))){
        ...
        p.sendMessage(Strings.servidormensagem + ChatColor.GRAY + "Essa senha nao foi encontrada , em nosso banco de dados .");
      }
    }
    return false;
  }
