...
public class Spawn implements CommandExecutor{
	...
	public boolean onCommand(CommandSender Sender, Command Cmd, String Label, String[] Args) {
		...
		if(Cmd.getName().equalsIgnoreCase("spawn")){
			...
			p.sendMessage(Strings.servidormensagem + ChatColor.GRAY + "Você está sendo " + ChatColor.YELLOW + ChatColor.BOLD + "TELEPORTADO" + ChatColor.GRAY + " para o Spawn!");
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getPlugin(), new Runnable() {
				...
				public void run() {
					  ...
					  WarpsAPI.Ir(p, "Spawn");
						API.setWarp(p, "Spawn");
						WarpsAPI.removerFPS();
						p.sendMessage(Strings.servidormensagem + ChatColor.GRAY + "Você foi " + ChatColor.YELLOW + ChatColor.BOLD + "TELEPORTADO" + ChatColor.GRAY + " com sucesso!");
						API.itensSpawn(p);
						KitAPI.TirarKit(p);
