	      ...
	      if (args.length > 0)
	      {
	        ...
	        for (int i = 0; i < j; i++)
	        {
	          ...
	          if (staff.hasPermission("cover.staffchat")){
	            staff.sendMessage(ChatColor.GRAY + "[" + ChatColor.GOLD + ChatColor.BOLD + "StaffChat" + ChatColor.GRAY + "] " + p.getDisplayName() + ChatColor.GRAY + " (" + Manager.prefix(staff) + ChatColor.GRAY + ")" + ChatColor.WHITE + " » §7" + mensagem);
	        }
	        }
	      }
	    }
