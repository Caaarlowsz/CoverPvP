					...
					if (args[0].toLowerCase().equals("#")) {
						if (FakeManager.fake.containsKey(player)) {
							try {
								...
								player.sendMessage(Strings.servidormensagem + ChatColor.GRAY + "Seu fake foi resetado!");
								return true;
							}
						} else {
						...
						if(FakeManager.fake.containsKey(player)) {
							player.sendMessage(Strings.servidormensagem + ChatColor.GRAY + "Você já está utilizando um fake! Remova-o com /fake #!");
							return true;
						}
						String nickname = args[0];
						if (!FakePlayerUtils.validateName(nickname)) {
							player.sendMessage(Strings.servidormensagem + ChatColor.GRAY + "O nickname escolhido possui caractéres inválidos!");
							return true;
						}
						if (FakeManager.fakes.contains(nickname)) {
							player.sendMessage(Strings.servidormensagem + ChatColor.GRAY + "Este fake já está em uso!");
							return true;
						}
						try {
							...
							player.sendMessage(ChatColor.GREEN + "Seu nickname foi alterado para " + nickname + "!");
						} catch (Exception exception) {
							player.sendMessage(Strings.servidormensagem + ChatColor.GRAY + "Seu nickname foi alterado para " + ChatColor.YELLOW + ChatColor.BOLD + nickname + "!");
							return true;
						}
					}
				}
