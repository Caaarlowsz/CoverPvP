...
public class Bc extends Manager{
	...
	public void Comando(CommandSender sender, Command command, String[] argumentos) {
		...
		if (command.getName().equalsIgnoreCase("bc") || (command.getName().equalsIgnoreCase("broadcast")) && (p.hasPermission("cover.bc"))) {
			if (argumentos.length >= 1) {
		        ...
		        Bukkit.broadcastMessage(ChatColor.DARK_RED + "" + ChatColor.BOLD + "BROADCAST " + ChatColor.WHITE + "» " + ChatColor.GRAY + bcast);
			} else {
