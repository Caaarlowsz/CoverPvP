		...
		if (cmd.getName().equalsIgnoreCase("groupyoutuber")) {
			...
			if (args.length == 0) {
				p.sendMessage(Strings.servidormensagem + ChatColor.GRAY
						+ "Use /groupyoutuber <player> <semiyt, youtuber, youtuber+>");
				return true;
			}
			if (args.length == 1) {
			...
			if (args.length == 2) {
				...
				if (vipgroup.equalsIgnoreCase("semiyt")) {
					p.sendMessage("");
					p.sendMessage(
							ChatColor.YELLOW + "Você setou " + ChatColor.GRAY + "[ " + ChatColor.YELLOW + ChatColor.BOLD
									+ "SEMIYT" + ChatColor.GRAY + " ] " + ChatColor.YELLOW + "Foi dado ao Jogador "
									+ jogador.getName() + ChatColor.YELLOW + "[" + jogador.getUniqueId() + "]");
					p.sendMessage("");
					Bukkit.dispatchCommand(Bukkit.getConsoleSender(),
							"pex user " + jogador.getName() + " group set Semiyt");
					jogador.setDisplayName("§e§lSEMIYT §e" + jogador.getName());
					NametagAPI.setPrefix(jogador.getName(), "§e§lSEMIYT §e");
					NametagAPI.setSuffix(jogador.getName(),
							"§7 " + ChatColor.GRAY + "[" + Manager.prefix(jogador) + ChatColor.GRAY + "]");
				...
				if (vipgroup.equalsIgnoreCase("youtuber")) {
					p.sendMessage("");
					p.sendMessage(
							ChatColor.YELLOW + "O Youtuber " + ChatColor.GRAY + "[ " + ChatColor.AQUA + ChatColor.BOLD
									+ "YOUTUBER" + ChatColor.GRAY + " ] " + ChatColor.YELLOW + "Foi dado ao Jogador "
									+ jogador.getName() + ChatColor.YELLOW + "[" + jogador.getUniqueId() + "]");
					p.sendMessage("");
					Bukkit.dispatchCommand(Bukkit.getConsoleSender(),
							"pex user " + jogador.getName() + " group set Youtuber");
				...
				if (vipgroup.equalsIgnoreCase("youtuber+")) {
					p.sendMessage("");
					p.sendMessage(ChatColor.YELLOW + "O Youtuber " + ChatColor.GRAY + "[ " + ChatColor.DARK_AQUA
							+ ChatColor.BOLD + "YOUTUBER+" + ChatColor.GRAY + " ] " + ChatColor.YELLOW
							+ "Foi dado ao Jogador " + jogador.getName() + ChatColor.YELLOW + "["
							+ jogador.getUniqueId() + "]");
					p.sendMessage("");
					Bukkit.dispatchCommand(Bukkit.getConsoleSender(),
							"pex user " + jogador.getName() + " group set Youtuber+");
