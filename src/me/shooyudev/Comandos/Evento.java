	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (p.hasPermission("cover.evento")) {
			if (cmd.getName().equalsIgnoreCase("evento")) {
				if (evento == false) {
					p.sendMessage(
							Strings.servidormensagem + ChatColor.GRAY + "O Evento esta fechado ou n�o esta ocorrendo.");
					return true;
				}
				if (participantes.contains(p.getName())) {
					p.sendMessage("�7Voc� ja esta no evento.");
					return true;
				}
				if (evento == true) {
					Prote��o.setImortalidade(p, true);
					p.sendMessage("�7Voc� entrou no evento.");
		...
		if (args[0].equalsIgnoreCase("iniciar")) {
			...
			Bukkit.broadcastMessage(" " + ChatColor.YELLOW + "Quer participar? D� /evento");
			Bukkit.broadcastMessage(
					" " + ChatColor.YELLOW + "Pr�mio: " + ChatColor.GREEN + "Receber� no final do evento.");
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(CoverPvP.instance, new Runnable() {
				...
				public void run() {
					if (participantes.size() >= 2) {
						...
						Bukkit.broadcastMessage(
								" " + ChatColor.YELLOW + "Pr�mio: " + ChatColor.GREEN + "Receber� no final do evento.");
						Prote��o.setImortalidade(p, false);
						evento = false;
						comecou = true;
					} else {
