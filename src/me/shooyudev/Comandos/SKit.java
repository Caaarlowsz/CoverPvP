...
public class SKit implements Listener, CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String CommandLabel, String[] args) {
		...
		if ((cmd.getName().equalsIgnoreCase("skit")) && (p.hasPermission("cover.skit"))) {
			if (args.length == 0) {
				p.sendMessage(
						Strings.servidormensagem + ChatColor.GRAY + "Use /skit <Criar> <aplicar> <nome> <distancia>");
				return true;
			}
			if (args[0].equalsIgnoreCase("criar")) {
				...
				p.sendMessage(
						Strings.servidormensagem + ChatColor.GRAY + "O kit " + args[1] + " foi criado com Sucesso!");
				return true;
			}
			if (args[0].equalsIgnoreCase("aplicar")) {
				...
				if ((!this.kit.containsKey(name)) && (!this.armadura.containsKey(name))) {
					p.sendMessage(Strings.servidormensagem + ChatColor.GRAY + "O kit " + name
							+ " se encontra inexistente no sistema!");
					return true;
				}
				if (isInt(args[2])) {
					...
					p.sendMessage(Strings.servidormensagem + ChatColor.GRAY + "O kit " + ChatColor.YELLOW
							+ ChatColor.BOLD + args[1] + ChatColor.GRAY + " foi aplicado para todos !");
					return true;
				}
			}
		}
