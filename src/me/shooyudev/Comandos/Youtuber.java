		...
		if (command.getName().equalsIgnoreCase("youtuber")) {
			...
			if (argumentos.length == 1) {
				if (argumentos[0].equalsIgnoreCase("yt")) {
					p.sendMessage(" ");
					p.sendMessage(ChatColor.GRAY + "Para se tornar um " + ChatColor.AQUA + ChatColor.BOLD + "YOUTUBER"
							+ ChatColor.GRAY + " do servidor .");
					...
					p.sendMessage(" ");
					p.sendMessage(
							ChatColor.GRAY + "INFO: " + ChatColor.GRAY + "Mande seu video para um cargo superior que "
									+ ChatColor.DARK_PURPLE + "MOD" + ChatColor.GRAY + "!");
					p.sendMessage(" ");
					return;
				}
				if (argumentos[0].equalsIgnoreCase("yt+")) {
					p.sendMessage(" ");
					p.sendMessage(ChatColor.GRAY + "Para se tornar um " + ChatColor.DARK_AQUA + ChatColor.BOLD
							+ "YOUTUBER+" + ChatColor.GRAY + " do servidor .");
				...
				if (argumentos[0].equalsIgnoreCase("semiyt")) {
					p.sendMessage(" ");
					p.sendMessage(ChatColor.GRAY + "Para se tornar um " + ChatColor.GOLD + ChatColor.BOLD + "SEMI-YT"
							+ ChatColor.GRAY + " do servidor .");
