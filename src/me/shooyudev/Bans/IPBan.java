	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("ipban")) {
			...
			if (vitima.getName().equals("TranquilliYT")) {
				API.sendMsg(jogador, Strings.servidormensagem + ChatColor.GRAY + "Você esta bloqueado de banir (ip) o " + ChatColor.DARK_RED + ChatColor.BOLD + "DONO" + ChatColor.DARK_RED + " TranquilliYT");
				API.sendSound(jogador, Sound.NOTE_PLING, 10);
				return true;
			}
			if (vitima.getName().equals("TranquilliYT")) {
			...
			API.sendMsg(jogador, ChatColor.GRAY + "Você baniu o (ip) jogador " + ChatColor.YELLOW + ChatColor.BOLD + vitima.getName() + ChatColor.DARK_RED + ChatColor.BOLD + " PERMANENTEMENTE" + ChatColor.GRAY + " .");
			API.sendStaff(ChatColor.GRAY + "O Staff " + ChatColor.YELLOW + ChatColor.BOLD + jogador.getName() + ChatColor.GRAY + " baniu (ip) do jogador " + ChatColor.YELLOW + ChatColor.BOLD + vitima.getName() + ChatColor.GRAY + ". Motivo: " + ChatColor.YELLOW + ChatColor.BOLD + Motivo);
			vitima.kickPlayer("§7Você foi §4§lPERMANENTEMENTE §7ip banido do servidor!" + "\n" + "\n" + "§7Username banido: §6§l" + vitima.getName() + "\n" + "§7IP banido: §6§l" + vitima.getAddress().getHostString() + "\n"+"§7Banido por: §6§l" + jogador.getName() + "\n" + "§7Motivo: §6§l" + Motivo + "\n" + "§7Vencimento : §6§lNUNCA" + "\n" + "§7Data do banimento: §6§l" + HorarioAPI.getHorario() + "\n" + "§7Compre seu §6§lUNBAN §7em nossa loja: §6§nloja-arkuzmc.buycraft.net");
