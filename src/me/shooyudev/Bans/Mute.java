	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("mute")) {
			...
			if (vitima.getName().equals("TranquilliYT")) {
				API.sendMsg(jogador, Strings.servidormensagem + ChatColor.GRAY + "Você esta bloqueado de Mutar o "
			...
			if (Config.getConfig().getBans().get("Mute." + vitima.getUniqueId()) != null) {
				API.sendMsg(jogador,
						Strings.servidormensagem + ChatColor.GRAY + "O jogador " + ChatColor.YELLOW + ChatColor.BOLD
								...
								+ "PERMANENTEMENTE" + ChatColor.GRAY + " Mutado !");
				API.sendSound(jogador, Sound.NOTE_PLING, 10);
				return true;
			}
			if (args.length == 1) {
			...
			API.sendMsg(jogador, Strings.servidormensagem + ChatColor.GRAY + "Você mutou o jogador " + ChatColor.YELLOW
					+ ChatColor.BOLD + vitima.getName() + ChatColor.GRAY + ".");
			API.sendStaff(Strings.servidormensagem + ChatColor.GRAY + "O Staff " + ChatColor.YELLOW + ChatColor.BOLD
					+ jogador.getName() + ChatColor.GRAY + " mutou o jogador " + ChatColor.YELLOW + ChatColor.BOLD
					+ vitima.getName() + ChatColor.GRAY + ". Motivo: " + ChatColor.YELLOW + ChatColor.BOLD + Motivo);
			API.sendMsg(vitima,
					Strings.servidormensagem + ChatColor.GRAY + "Você foi Mutado por: " + ChatColor.YELLOW
							+ ChatColor.BOLD + jogador.getName() + ChatColor.GRAY + ". Motivo: " + ChatColor.YELLOW
							+ ChatColor.BOLD + Motivo);
